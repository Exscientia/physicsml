...
_DESCRIPTION = """
A representation is generated from the latent layers of the Allegro model
"""


class AllegroFeaturiser(PrismLightningBase):
    ...
    def forward(self, x: Dict[str, Any]) -> Any:
        ...
        edge_feats = x["edge_feats"]
        node_feats = scatter(
            src=edge_feats,
            index=x["edge_index"][0],
            dim=0,
            dim_size=int(x["num_nodes"]),
        )
        node_feats = node_feats[:, : self.module.irreps_layer_out[0].dim]
        batch = x["batch"]
...
class Allegro(PhysicsMLPrismBase):
    def __init__(
        self,
        ...
        **kwargs: Any,
        ...
        self.model.module = AllegroFeaturiser(
            module=self.model.module.allegro,
            which_rep=which_rep,
        )

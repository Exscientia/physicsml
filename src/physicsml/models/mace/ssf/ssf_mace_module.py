...
from physicsml.models.mace.ssf.default_configs import SSFMACEModelConfig
from physicsml.models.mace.ssf.ssf_mace_utils import SSFMACE


class PooledSSFMACEModule(PhysicsMLModuleBase):
    """
    Class for pooled ssf mace model
    """

    model_config: SSFMACEModelConfig
        ...
        **kwargs: Any,
        ...
        if model_config.datamodule.y_node_scalars is not None:
            self.node_scalars_head: Optional[torch.nn.Module] = ReadoutHead(
                list_in_irreps=self.ssf_mace.out_irreps,
    ...
    def configure_optimizers(self) -> Dict:  # type: ignore
        ...
        # Optimizer
        decay_interactions = []
        for name, _ in self.ssf_mace.interactions.named_parameters():
            if "linear.weight" in name or "residual_connection_layer.weight" in name:
                decay_interactions.append(name)

        for name, _ in self.ssf_mace.messages.named_parameters():
            if "linear.weight" in name or "residual_connection_layer.weight" in name:
                decay_interactions.append(name)

        for name, _ in self.ssf_mace.node_updates.named_parameters():
            if "linear.weight" in name or "residual_connection_layer.weight" in name:
                decay_interactions.append(name)

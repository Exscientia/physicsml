...
from physicsml.models.mace.adapter.adapter_mace_utils import AdapterMACE
from physicsml.models.mace.adapter.default_configs import AdapterMACEModelConfig
from physicsml.models.mace.modules.mace import PooledReadoutHead, ReadoutHead


class PooledAdapterMACEModule(PhysicsMLModuleBase):
    """
    Class for pooled adapter mace model
    """

    model_config: AdapterMACEModelConfig
        ...
        **kwargs: Any,
        ...
        if model_config.datamodule.y_node_scalars is not None:
            self.node_scalars_head: Optional[torch.nn.Module] = ReadoutHead(
                list_in_irreps=self.adapter_mace.out_irreps,
    ...
    def configure_optimizers(self) -> Dict:  # type: ignore
        ...
        # Optimizer
        decay_interactions = []
        for name, _ in self.adapter_mace.interactions.named_parameters():
            if "linear.weight" in name or "residual_connection_layer.weight" in name:
                decay_interactions.append(name)

        for name, _ in self.adapter_mace.messages.named_parameters():
            if "linear.weight" in name or "residual_connection_layer.weight" in name:
                decay_interactions.append(name)

        for name, _ in self.adapter_mace.node_updates.named_parameters():
            if "linear.weight" in name or "residual_connection_layer.weight" in name:
                decay_interactions.append(name)

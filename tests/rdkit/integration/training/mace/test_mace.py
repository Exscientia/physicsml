...
def test_training_mace(featurised_gdb9_atomic_nums_and_feats_and_bond_feats):
    (
        dataset_feated,
        x_features,
    ...
    assert "mace_model::u0" in preds.keys()
    assert len(preds) == 1
    assert len(preds["mace_model::u0"]) == 100

    assert (
        (torch.tensor(preds["mace_model::u0"]) - torch.tensor(dataset_feated["u0"]))
        ** 2
    ).mean().sqrt() < 1.0

...
def test_training_nequip(featurised_gdb9_atomic_nums_and_feats_and_bond_feats):
    (
        dataset_feated,
        x_features,
    ...
    assert "nequip_model::u0" in preds.keys()
    assert len(preds) == 1
    assert len(preds["nequip_model::u0"]) == 100

    assert (
        (torch.tensor(preds["nequip_model::u0"]) - torch.tensor(dataset_feated["u0"]))
        ** 2
    ).mean().sqrt() < 1.0

...
def test_training_adapter_nequip(featurised_gdb9_atomic_nums_and_feats_and_bond_feats):
    (
        dataset_feated,
        x_features,
    ...
    model_config = {
        "name": "adapter_nequip_model",  # the model name
        "config": {
    ...
    assert "adapter_nequip_model::u0" in preds.keys()
    assert len(preds) == 1
    assert len(preds["adapter_nequip_model::u0"]) == 100

    assert (
        (
            torch.tensor(preds["adapter_nequip_model::u0"])
